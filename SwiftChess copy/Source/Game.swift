...
public final class Game {
    ...
    public enum State: Equatable {
        case inProgress
        case staleMate(color: ChessColor)
        case won(color: ChessColor)
    ...
    public enum Outcome: Hashable, CustomStringConvertible {
        ...
        internal static func _win(_ color: ChessColor) -> Outcome {
            #if swift(>=3)
                return .win(color)
            #else
                return .Win(color)
        ...
        public var winColor: ChessColor? {
            #if swift(>=3)
        ...
        public func value(for playerColor: ChessColor) -> Double {
...
extension Game.State: DictionaryRepresentable {
    ...
    init?(dictionary: [String: Any]) {
        ...
        case Keys.type_stalemate:
            guard let raw = dictionary[Keys.color] as? String, let color = ChessColor(rawValue: raw) else {
                return nil
            }
...
extension Game: DictionaryRepresentable {
    ...
    public convenience init?(dictionary: [String: Any]) {
        ...
        guard let stateDict = dictionary[Keys.state] as? [String: Any],
            ...
            let currentPlayerColor = ChessColor(rawValue: currentPlayerColorRaw)
            else {
